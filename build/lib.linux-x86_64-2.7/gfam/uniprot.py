...
class XMLIprscanToTxt(object):
        ...
        fields = ["protein_id", "protein_crc64", "protein_length", "match_evd",
                "match_id", "match_name", "lcn_start", "lcn_end", "lcn_score",
                "match_status", "date", "ipr_id", "ipr_name"]
            ...
            if tag[0] == '<' and tag[1] != '/' and tag.endswith(">"):
                ...
                if self.header_is_read:
                    ...
                    for result in re.finditer(self.attr, content):
                        key, val = result.group().strip().replace("\"", "").split("=")
                        self.match_data[tag_name + "_" + key.strip()] = val.strip()
                ...
                print "ERROR in XML file, tag=", tag
                sys.exit(-1)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 6:
        print XMLIprscanToTxt.__doc__.replace("%prog", sys.argv[0])
        sys.exit(-1)

    scanner = XMLIprscanToTxt(*sys.argv[1:])
    scanner.run()
