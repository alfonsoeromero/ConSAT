...
There are four use cases in this program:
    ...
    1.- Run GFam/ConSAT in UniProt: all the related UniProtKB files will be
    downloaded, including the InterPro data. The user will only have to
    ...
    rest (TremBL). The models learnt by GFam in SwissProt will be used by
    ConSAT although the user can include a previously found set of models
    of putative new domains to extend this and maintain the names with respect
    to the old domains. The putative models found will be named CPPD00001,
    CPPD00002, etc. It is assumed that the provided models are also following
    ...
    inputing. A set of putative new domain models will be predicted for this
    set of sequences (NOVEL00001, NOVEL00002, etc.). This is equivalent to the
...
class AutomatedConSAT(CommandLineApp):
    ...
    1.- Run GFam/ConSAT on UniProt: all the related UniProtKB files will be
    downloaded, including the InterPro data. The user will only have to provide
    a directory where the downloaded and the output files will be written.

    GFam will be run in the SwissProt proteins, and ConSAT will be run in the
    rest (TremBL). The models learnt by GFam in SwissProt will be used by
    ConSAT although the user can include a previously found set of models
    of putative new domains to extend this and maintain the names with
    respect to the old domains. The putative models found will be
    named CPPD00001, CPPD00002, etc.
    ...
    All four cases will take a single positional argument, a route to
    a directory which will be created if it does not exists, and where
    three sub-directories will be setup:
    ...
    The GFam configuration file used will be created in the root directory
    of the specfied route.

    In the first use case (UniProt), two configuration files (gfam.cfg
    and consat.cfg) will be created, and one additional directory level
    will be setup in this case:
    ...
    urls = {"file.mapping.gene_ontology":
    ...
    def create_parser(self):
        ...
        parser.add_option("-t", "--data", dest="data_dir",
                          help="provides a certain (existing) "
                               "data dir where the files will "
                               "be taken",
                               metavar="DIR")
        ...
        parser.add_option("-m", "--models", dest="models",
                          metavar="FILE",
                          help="file with previously computed HMM models"
                               "to maintain nomenclature (uniprot/gfam mode)"
                               " or use them (consat mode)")
        parser.add_option("-s", "--user-sequences", dest="sequences",
                          metavar="FILE",
                          help="file with user sequences (gfam/consat modes)")
        ...
        parser.add_option("-r", "--regexp", dest="regexp", metavar="REGEX",
                          help="regular expression to extract protein "
                               "ids (gfam/consat)")
        parser.add_option("-n", "--no-download", dest="no_download",
                          action="store_true",
                          help="does not download data, use if you"
                               " want to continue a previous execution")
        parser.add_option("-g", "--goa", dest="goa",
                          help="GOA file to transfer the function from")
        parser.add_option("-b", "--blast-route", dest="blast_route",
                          help="route to blast executables if not in the path")
        parser.add_option("-p", "--pubmed-cache", dest="pubmed_cache",
                          help="route to the cache of PubMed downloaded files")
        parser.add_option("-o", "--gene-ontology",
                          dest="gene_ontology",
                          help="route to the desired Gene Ontology file"
                               " (instead of using the most recent one")
        parser.add_option("-x", "--lexicon", dest="lexicon",
                          help="route to lexicon "
    ...
    def process_arguments(self):
        if len(self.args) != 2:
            self.error("Wrong argument number. "
                       "Command executed: {}".format(" ".join(self.args)))

        self.mode, self.directory = self.args

        if self.mode not in ["uniprot", "uniprot_consat", "gfam", "consat"]:
            self.error("Wrong mode, must be 'uniprot', 'uniprot_consat',"
                       " 'gfam' or 'consat'")
    ...
    def init_folders(self):
        ...
        for dirname in ["data", "work", "output"]:
            if dirname == "data" and self.options.data_dir:
                ...
                self.program_dir[dirname] = os.path.join(self.directory,
                                                         dirname)
        ...
        if self.options.pubmed_cache:
            ...
            prog_data = self.program_dir["data"]
            self.params["folder.pubmed_cache"] = os.path.join(prog_data,
                                                              "pubmed_cache")
    ...
    def download_data(self):
        ...
        for name, url in self.urls.items():
            ...
            if name != "file.mapping.gene_ontology"\
               or (name == "file.mapping.gene_ontology"
                   and not self.options.gene_ontology):
                self._download_file(url, filename)
                self.params[name] = filename

        if self.options.gene_ontology:
            opt_go = self.options.gene_ontology
            self.params["file.mapping.gene_ontology"] = opt_go
        ...
        if self.mode == "uniprot":
            ...
            url_swissprot = "ftp://ftp.ebi.ac.uk/pub/databases/" +\
                            "uniprot/knowledgebase/uniprot_sprot.fasta.gz"
            file_swissprot = os.path.join(self.program_dir["data"],
                                          "uniprot_sprot.fasta.gz")
            self._download_file(url_swissprot, file_swissprot)
            self._gunzip(file_swissprot)
            file_swissprot = file_swissprot.replace(".gz", "")

            url_trembl = "ftp://ftp.ebi.ac.uk/pub/databases/uniprot/" +\
                         "knowledgebase/uniprot_trembl.fasta.gz"
            file_trembl = os.path.join(self.program_dir["data"],
            ...
            file_sprot_ids = os.path.join(self.gfam_dir["data"],
                                          "swissprot_ids")
            with open(file_sprot_ids, "w") as out:
                f_input_seqs = self.params_gfam["file.input.sequences"]
                for line in open(f_input_seqs, "r"):
                    if line.startswith(">"):
            ...
            interpro_gfam = os.path.join(self.gfam_dir["data"],
                                         "uniprot_sprot.interpro")
            self.params_gfam["file.input.iprscan"] = interpro_gfam
            interpro_consat = os.path.join(self.consat_dir["data"],
                                           "uniprot_trembl.interpro")
            ...
            if not os.path.isfile(interpro_gfam)\
               and not os.path.isfile(interpro_consat):
                conversor = XMLIprscanToTxt(file_interpro,
                                            self.params[
                                                "file.mapping.interpro2go"],
                                            file_sprot_ids,
                                            interpro_gfam,
            ...
            computed_models = os.path.join(self.params_gfam["folder.output"],
                                           "hmms")
            ...
            old_lexicon = os.path.join(self.params_gfam["folder.output"],
                                       "lexicon")
            self.params_consat["file.lexicon"] = old_lexicon
            table = os.path.join(self.params_gfam["folder.output"],
            ...
            url_rdf = "http://www.uniprot.org/citations/" +\
                      "?query=citedin%3a(*)&compress=yes&format=rdf"
            rdf_file = os.path.join(self.program_dir["data"],
                                    "citations.rdf.gz")
            ...
            url_id_mapping = "ftp://ftp.uniprot.org/pub/databases/uniprot/" +\
                             "current_release/knowledgebase/idmapping/" +\
                             "idmapping_selected.tab.gz"
            id_mapping_file = os.path.join(self.program_dir["data"],
                                           "idmapping_selected.tag.gz")
            ...
            if self.options.goa:
                ...
                name_goa = os.path.basename(self.options.goa)
                self.params["file.function.goa_file"] = os.path.join(data,
                                                                     name_goa)
            ...
            self.log.info("Downloading sequences...")
            url_swissprot = "ftp://ftp.ebi.ac.uk/pub/databases/uniprot" +\
                            "/knowledgebase/uniprot_sprot.fasta.gz"
            file_swissprot = os.path.join(self.program_dir["data"],
                                          "uniprot_sprot.fasta.gz")
            self._download_file(url_swissprot, file_swissprot)
            self._gunzip(file_swissprot)
            ...
            # we write in a file the set of swissprot ids
            file_sprot_ids = os.path.join(self.program_dir["data"],
                                          "swissprot_ids")
            with open(file_sprot_ids, "w") as out:
            ...
            interpro_out = os.path.join(self.program_dir["data"],
                                        "uniprot.interpro")

            # Conversion of the intepro file (XML) into text
            if not os.path.isfile(interpro1) and not os.path.isfile(interpro2):
                fl_ipr2go = self.params["file.mapping.interpro2go"]
                conversor = XMLIprscanToTxt(file_interpro,
                                            fl_ipr2go,
                                            file_sprot_ids,
                                            interpro1,
    ...
    def join_text_representation(self):
        self.log.info("Joining UniProtKB (sp + tr) textual"
                      " annotation in a single file")

        N_sp = self.get_num_proteins(self.params_gfam["file.input.sequences"])
        N_tr = self.get_num_proteins(self.params_consat[
        ...
        lexicon_tr = os.path.join(self.params_consat["folder.output"],
                                  "lexicon")
        ...
        text_file_sp = os.path.join(self.params_gfam["folder.output"],
                                    "weight_file_per_arch")
        text_file_tr = os.path.join(self.params_consat["folder.output"],
                                    "weight_file_per_arch")
        out_file = os.path.join(self.params_consat["folder.output"],
                                "weight_file_per_arch_sp_tr")
        ...
        with open(out_file, "w") as out:
            for _line in open(text_file_tr):
                ...
                out.write(arch + " ")
                out.write(" ".join(["{0}:{1}".format(id, w)
                                    for id, w in v_idf.items()]))
                out.write("\n")
                visited.add(arch)
