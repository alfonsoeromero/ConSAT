#!/usr/bin/env python
"""Application that takes a clustering file and outputs a new domains table
"""
...
class BuildNewDomains(CommandLineApp):
    """\
    Usage: %prog [options] clustering_file

    Application that takes the clustering file, discard useless clusters
    and name those useful ones as new domains, printing a tab separated file,
    with the new domain names as the first string, and the sequence fragments
    after it
    """

    short_name = "build_new_domains"
    ...
    def create_parser(self):
        ...
        parser.add_option("-s", "--min-size", dest="min_size",
                ...
                config_key="analysis:build_new_domains/min_novel_domain_size",
                default=2, type=int)
    ...
    def run_real(self):
        ...
        if len(self.args) != 1:
            self.error("exactly one input files are expected")
